...
export async function POST(request: Request) {
    try {
        ...
        async start(controller) {
            ...
            controller.enqueue(encoder.encode(`data: ${JSON.stringify({ type: 'finalMessage', message: finalMessage.content[0].type === 'text' ? finalMessage.content[0].text : '' })}\n\n`));
            
            const { data, error } = await supabase
                .from('conversations')
                .insert([
                    { prompt: command, response: fullResponse }
                ]);
